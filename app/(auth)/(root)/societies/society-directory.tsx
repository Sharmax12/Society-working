...
export function SocietyDirectory({ societies }: { societies: Society[] }) {
  ...
  return <>
    ...
    <div className="mx-auto max-w-7xl px-5 py-12 sm:px-8 lg:px-10">
      ...
      {filteredSocieties.length === 0 ? <div className="rounded-[2rem] border border-dashed p-20 text-center text-muted-foreground"><Search className="mx-auto mb-4 h-7 w-7 opacity-50" /><p className="font-semibold text-foreground">No societies match your search.</p><p className="mt-1 text-sm">Try a broader interest, category, or society name.</p>{search && <button type="button" onClick={() => setSearch("")} className="mt-5 rounded-xl bg-foreground px-4 py-2.5 text-xs font-bold text-background">Clear search</button>}</div> : <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">{filteredSocieties.map((society, index) => <Link key={society.id} href={`/societies/${society.id}`} className="group relative overflow-hidden rounded-[1.6rem] border border-foreground/10 bg-card p-6 transition-all duration-300 hover:-translate-y-1 hover:border-primary/30 hover:shadow-2xl hover:shadow-primary/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><div className="absolute right-0 top-0 h-28 w-28 rounded-full bg-primary/5 blur-2xl transition group-hover:bg-primary/10" /><div className="relative"><div className="mb-8 flex items-start justify-between"><div className="grid h-12 w-12 place-items-center rounded-2xl bg-primary/10 text-primary"><Users className="h-5 w-5" /></div><span className="text-[10px] font-black text-muted-foreground">{String(index + 1).padStart(2, "0")}</span></div>{society.category && <Badge variant="secondary" className="mb-3 rounded-full capitalize">{society.category}</Badge>}<h3 className="text-xl font-black tracking-tight">{society.name}</h3><p className="mt-2 line-clamp-3 text-sm leading-6 text-muted-foreground">{society.description}</p><div className="mt-7 flex items-center justify-between border-t border-foreground/10 pt-4 text-xs text-muted-foreground"><span>Closes {new Date(society.deadline).toLocaleDateString("en-US", { month: "short", day: "numeric" })}</span><span className="flex items-center gap-1 font-bold text-foreground">View <ArrowUpRight className="h-3.5 w-3.5 transition-transform group-hover:-translate-y-0.5 group-hover:translate-x-0.5" /></span></div></div></Link>)}</div>}
    </div>
  </>;
}
